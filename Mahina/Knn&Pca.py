...
class KNNCentroidsPlotter:
    ...
    def preprocess_data(self):
        X = self.df[['LPG Level (PPM)', 'Time']].values
        y = self.df['Situations']
    ...
    def plot_centroids(self):
        x_min, x_max = self.df['LPG Level (PPM)'].min() - 1, self.df['LPG Level (PPM)'].max() + 1
        time_min, time_max = self.df['Time'].min() - 1, self.df['Time'].max() + 1
    ...
    def run(self, k=3, colors=None):
        ...
        X_test = self.df[['LPG Level (PPM)', 'Time']].values
        y_test = self.label_encoder.transform(self.df['Situations'])
